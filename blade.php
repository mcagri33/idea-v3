...
@section('content')
  ...
  <div class="d-flex gap-2 mb-4">
    <a href="{{ route('general.note.exportPdf', ['user' => $user->id, 'year' => $year]) }}" 
    ...
    <div class="table-responsive text-nowrap">
      <table class="table" id="categoriesTable">
        ...
        @foreach($categories as $category)
          @php
            $categoryNote = $category->notes()->where('user_id', $user->id)->first();
          @endphp
          <tr>
            <td>{{ $category->name }}</td>
...
<script>
  ...
  async function saveCategoryNote(categoryId, userId) {
    ...
    try {
      const response = await fetch('{{ route("documents.status.note.update") }}', {
        method: 'POST',
        headers: {
